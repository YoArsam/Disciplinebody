...
function HabitAdder({ habit, onSave, onDelete, onBack }) {
  ...
  const [habitTime, setHabitTime] = useState(habit?.habitTime || '') // HH:mm or empty
  const [hasHabitTime, setHasHabitTime] = useState(habit?.habitTime ? true : false)
  ...
  const canGoNext = () => {
    ...
    if (step === 2) return !hasHabitTime || !!habitTime
  ...
  const handleSubmit = (e) => {
    ...
    onSave({
      ...(habit || {}),
      name: name.trim(),
      habitTime: hasHabitTime ? habitTime : '',
      ...
      <div className={`flex-1 overflow-y-auto min-h-0 ${onDelete ? 'pb-48' : 'pb-36'} space-y-4`}>
        <form id="habit-form" onSubmit={handleSubmit} className="space-y-4">
          ...
          {(isEditing || step === 2) && (
            <div className="bg-white border border-gray-200 rounded-[1.5rem] p-5">
              <div className="mb-4">
                <span className="text-lg font-bold text-gray-900">Habit Time</span>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setHasHabitTime(true);
                      if (!habitTime) setHabitTime('09:00'); // Default to 9 AM
                    }}
                    className={`flex-1 h-12 rounded-xl font-bold text-sm border transition-all active:scale-95 ${
                      hasHabitTime 
                        ? 'bg-orange-500 text-white border-orange-500' 
                  ...
                  <div className="animate-fadeIn w-full overflow-hidden">
                    <div className="relative w-full flex justify-center">
                      <input
                        ...
                        className="w-full max-w-full bg-gray-50 text-gray-900 rounded-2xl py-6 px-2 text-center text-4xl font-black focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all border border-gray-100"
                        style={{ minHeight: '100px', appearance: 'none', WebkitAppearance: 'none' }}
                      />
                    </div>
